...
# If we were given an argument then that was a license path and we need to do something with it
if [ "$1" != "" ]; then
    ...
    echo $1
    echo 'if ( $?ARMLMD_LICENSE_FILE ) then'                            >  $DIR/license.csh
    echo '    if ( "$ARMLMD_LICENSE_FILE" =~ ?* ) then'                 >> $DIR/license.csh
    echo "        setenv ARMLMD_LICENSE_FILE \"$1:\$ARMLMD_LICENSE_FILE\""  >> $DIR/license.csh
    echo '    else'                                                 >> $DIR/license.csh
    echo "        setenv ARMLMD_LICENSE_FILE \"$1\""                    >> $DIR/license.csh
    echo '    endif'                                                >> $DIR/license.csh
    echo 'else'                                                     >> $DIR/license.csh
    echo "    setenv ARMLMD_LICENSE_FILE \"$1\""                        >> $DIR/license.csh
    echo 'endif'                                                    >> $DIR/license.csh
    
    echo 'if [ -z $ARMLMD_LICENSE_FILE ]; then'                         >  $DIR/license.sh
    echo "    export ARMLMD_LICENSE_FILE=\"$1\""                        >> $DIR/license.sh
    echo 'else'                                                     >> $DIR/license.sh
    echo "    export ARMLMD_LICENSE_FILE=\"$1:\$ARMLMD_LICENSE_FILE\""      >> $DIR/license.sh
    echo 'fi'                                                       >> $DIR/license.sh
fi

echo ""
echo "You now have to run the following command in order to setup your"
echo "environment to correctly run the Fast Model Portfolio from ARM."
...
echo "environment to correctly run the Fast Model Portfolio from ARM."  >> $POSTINSTLOG
