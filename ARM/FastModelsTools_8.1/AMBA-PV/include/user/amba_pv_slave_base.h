/*
 * AMBA-PV: amba_pv_slave_base.h - Base class for all AMBA-PV slave modules.
 *
 * Copyright 2007-2012 ARM Limited.
 * All rights reserved.
 */

#ifndef AMBA_PV_SLAVE_BASE__H
#define AMBA_PV_SLAVE_BASE__H

/**
 * @file        amba_pv_slave_base.h
 *
 ...
 *
 * This version of the method translates the blocking transport call into
 * amba_pv_if user-layer calls.
...
template<unsigned int BUSWIDTH>
inline void
amba_pv_slave_base<BUSWIDTH>::b_transport(int socket_id,
    ...
    if (trans.is_read()) {
        ...
        if (ex->get_length() == 1) {
            ...
            ex->set_resp(this->read(socket_id,
                                    trans.get_address(),
                                    trans.get_data_ptr(),
                                    ex->get_size(),
                                    ex,
         ...
         * size! */
        if ((trans.get_byte_enable_ptr() != NULL)
            && (trans.get_byte_enable_length() % ex->get_size())) {
            trans.set_response_status(tlm::TLM_BYTE_ENABLE_ERROR_RESPONSE);
            ex->set_slverr();
            return;
        }
 ...
 *
 * This version of the method translates the transport_dbg() call into
 * amba_pv_if user-layer calls.
 */
template<unsigned int BUSWIDTH>
inline unsigned int
amba_pv_slave_base<BUSWIDTH>::transport_dbg(int socket_id,
                                            amba_pv_transaction & trans) {
    amba_pv_extension * ex = NULL;
    ...
    if (trans.is_read()) {
        ...
        return (this->debug_read(socket_id,
                                 trans.get_address(),
                                 trans.get_data_ptr(),
        ...
        return (this->debug_write(socket_id,
                                  trans.get_address(),
                                  trans.get_data_ptr(),
                                  trans.get_data_length(),
                                  ex));
    } else {
        SC_REPORT_WARNING("amba_pv_slave_base",
                          "transport_dbg(): unsupported command");
        return 0;
    }
}
 ...
 *
 * This version of the method translates the DMI access request call into
 * amba_pv_if user-layer calls.
 */
template<unsigned int BUSWIDTH>
inline bool
amba_pv_slave_base<BUSWIDTH>::get_direct_mem_ptr(int socket_id,
                                                 amba_pv_transaction & trans,
    ...
    if (ex == NULL) {
        SC_REPORT_WARNING("amba_pv_slave_base",
                          "get_direct_mem_ptr(): NULL amba_pv_extension pointer");
        return false;
    }
    ...
    switch (trans.get_command()) {
        ...
        default:
            SC_REPORT_WARNING("amba_pv_slave_base",
                              "get_direct_mem_ptr(): unsupported command");
            return false;
    }
    return (this->get_direct_mem_ptr(socket_id,
                                     trans.get_command(),
                                     trans.get_address(),
                                     ex,
                                     dmi_data));
...
amba_pv_slave_base<BUSWIDTH>::burst_read(int socket_id,
    ...
    for (unsigned int n = 0; (n < length); n += 1) {
        ...
        /* Perform transfer */
        amba_pv_resp_t r = read(socket_id,
                                a,
                                data + (size * n),
                                size,
                                ctrl,
 ...
 *
 * This version of the method returns @c false and denies DMI access to the
...
#endif  /* defined(AMBA_PV_SLAVE_BASE__H) */
